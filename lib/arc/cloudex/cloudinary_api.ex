defmodule Arc.Cloudex.CloudinaryApi do
  @moduledoc """
  ...
  @json_library Application.get_env(:arc, :cloudex_json_library, Jason)
  ...
  """
  @spec delete(String.t(), map) :: {:ok, %Arc.Cloudex.DeletedImage{}} | {:error, any}
  def delete(item, opts \\ %{})

  def delete(item, opts) when is_bitstring(item) do
    case delete_file(item, opts) do
      {:ok, response} -> {:ok, %Arc.Cloudex.DeletedImage{public_id: item, response: response}}
      error -> error
    end
  end
  ...
  """
  @spec json_result_to_struct(map, String.t()) :: %Arc.Cloudex.UploadedImage{}
  def json_result_to_struct(result, source) do
    converted = Enum.map(result, fn {k, v} -> {String.to_atom(k), v} end) ++ [source: source]
    struct(%Arc.Cloudex.UploadedImage{}, converted)
  end

  @spec upload_file(String.t(), map) :: {:ok, %Arc.Cloudex.UploadedImage{}} | {:error, any}
  ...
  @spec upload_url(String.t(), map) :: {:ok, %Arc.Cloudex.UploadedImage{}} | {:error, any}
  ...
  defp credentials do
    [
      hackney: [
        basic_auth: {Arc.Cloudex.Settings.get(:api_key), Arc.Cloudex.Settings.get(:secret)}
      ]
    ]
  end
  ...
  defp delete_url_for(opts, item) do
    "#{@base_url}#{Arc.Cloudex.Settings.get(:cloud_name)}/resources/#{
  ...
  defp delete_prefix_url(resource_type, prefix) do
    "#{@base_url}#{Arc.Cloudex.Settings.get(:cloud_name)}/resources/#{resource_type}/upload?prefix=#{
      prefix
    }"
  end

  @spec post(tuple | String.t(), binary, map) :: {:ok, %Arc.Cloudex.UploadedImage{}} | {:error, any}
  ...
  def url(resource_type) do
    "#{@base_url}#{Arc.Cloudex.Settings.get(:cloud_name)}/#{resource_type}/upload"
  end

  @spec handle_response(map, String.t()) :: {:error, any} | {:ok, %Arc.Cloudex.UploadedImage{}}
  ...
  defp sign(data) do
    ...
    signature = sha(data_without_secret <> Arc.Cloudex.Settings.get(:secret))
    ...
    Map.merge(
      ...
      %{
        "timestamp" => timestamp,
        "signature" => signature,
        "api_key" => Arc.Cloudex.Settings.get(:api_key)
      }
    )
  end
